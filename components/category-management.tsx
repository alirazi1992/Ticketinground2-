...
export function CategoryManagement({ categoriesData, onCategoryUpdate }: CategoryManagementProps) {
  ...
  const [newField, setNewField] = useState<{
    ...
    placeholder?: string
    optionsText?: string
  }>({ id: "", label: "", type: "text", required: false, placeholder: "", optionsText: "" })
  ...
  const handleUpdateCategory = () => {
    ...
    if (newId !== originalId && categoriesData[newId]) {
      toast({ title: "خطا", description: "شناسه جدید تکراری است", variant: "destructive" })
      return
    }
    ...
    if (!current) {
      toast({ title: "خطا", description: "دسته‌بندی پیدا نشد", variant: "destructive" })
      return
    }
  ...
  const handleUpdateSubCategory = () => {
    ...
    const updatedCategories: any = { ...categoriesData }
    const subIssues = { ...(updatedCategories[selectedCategory]?.subIssues || {}) }
    ...
    if (newId !== originalId && subIssues[newId]) {
      toast({ title: "خطا", description: "شناسه زیر دسته تکراری است", variant: "destructive" })
      return
    }

    const current = subIssues[originalId]
    if (!current) {
      toast({ title: "خطا", description: "زیر دسته پیدا نشد", variant: "destructive" })
      return
    }
  ...
  const openFieldDesigner = (subId: string) => {
    ...
    const current = categoriesData[selectedCategory]?.subIssues?.[subId]?.fields || []
  ...
  const addNewField = () => {
    if (!newField.id || !newField.label) {
      toast({ title: "خطا", description: "شناسه و عنوان فیلد الزامی است", variant: "destructive" })
      return
    }
    if (editingFields.some((f) => f.id === newField.id)) {
      toast({ title: "خطا", description: "شناسه فیلد تکراری است", variant: "destructive" })
      return
    }
    const options = newField.type === "select" || newField.type === "radio" ? parseOptions(newField.optionsText || "") : []
    ...
    setNewField({ id: "", label: "", type: "text", required: false, placeholder: "", optionsText: "" })
    toast({ title: "موفق", description: "فیلد اضافه شد" })
  }

  const updateField = (index: number, patch: Partial<FormFieldDef> & { optionsText?: string }) => {
  ...
  return (
    <div className="space-y-6" dir="rtl">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-1">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation()
                          setEditingCategory({ ...category, id: key, originalId: key })
                        }}
                        className="font-iran"
                      >
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-1">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => setEditingSubCategory({ ...subCategory, id: key, originalId: key })}
                            className="font-iran"
                          >
                            <Edit className="w-3 h-3" />
                          </Button>
      ...
      <Dialog open={fieldDesignerOpen} onOpenChange={setFieldDesignerOpen}>
        <DialogContent className="max-w-3xl font-iran" dir="rtl" aria-describedby={undefined}>
          <DialogHeader>
            <DialogTitle className="text-right font-iran">طراحی فیلدهای سفارشی</DialogTitle>
          </DialogHeader>
          <div className="space-y-6">
            <div className="space-y-3">
              {editingFields.length === 0 && (
                <p className="text-sm text-muted-foreground text-right">هیچ فیلدی تعریف نشده است.</p>
              )}
              {editingFields.map((f, idx) => {
                const optionsText = (f.options || []).map((o) => `${o.value}:${o.label}`).join(", ")
                return (
                  <div key={f.id} className="grid grid-cols-12 gap-2 items-end border rounded-md p-3">
                    <div className="col-span-12 md:col-span-3 space-y-1">
                      <Label className="text-right">شناسه</Label>
                      <Input value={f.id} disabled className="text-right bg-muted" dir="rtl" />
                    </div>
                    <div className="col-span-12 md:col-span-3 space-y-1">
                    ...
                    <div className="col-span-12 md:col-span-2 space-y-1">
                      ...
                      <Select value={f.type} onValueChange={(v) => updateField(idx, { type: v as any })} dir="rtl">
                    ...
                    {(f.type === "select" || f.type === "radio") && (
                      <div className="col-span-12 md:col-span-6 space-y-1">
                        <Label className="text-right">گزینه‌ها (value:label, جداشده با کاما)</Label>
                        <Input
                          value={optionsText}
                    ...
                    <div className="col-span-12 md:col-span-12 flex justify-end">
                      <Button variant="ghost" className="text-red-600" onClick={() => removeField(idx)}>
                        حذف
                      </Button>
                    </div>
                  </div>
            ...
            <div className="border rounded-lg p-3 space-y-3">
              ...
              <div className="grid grid-cols-12 gap-2 items-end">
                <div className="col-span-12 md:col-span-3 space-y-1">
                  ...
                  <Input
                    value={newField.id}
                    onChange={(e) => setNewField((p) => ({ ...p, id: e.target.value }))}
                    className="text-right"
                    dir="rtl"
                ...
                <div className="col-span-12 md:col-span-2 space-y-1">
                  <Label className="text-right">نوع</Label>
                  <Select value={newField.type} onValueChange={(v) => setNewField((p) => ({ ...p, type: v as any }))} dir="rtl">
                    <SelectTrigger className="text-right">
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setFieldDesignerOpen(false)} className="font-iran">
                انصراف
              </Button>
